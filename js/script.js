...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Menu mobile toggle - VERSIÓN CORREGIDA
    ...
    if (hamburger && navLinks) {
        ...
        // SOLUCIÓN: Eliminar event listeners previos y agregar solo uno
        // Clonar el elemento para eliminar listeners previos
        const newHamburger = hamburger.cloneNode(true);
        hamburger.parentNode.replaceChild(newHamburger, hamburger);
        
        // UN SOLO event listener para el hamburger - VERSIÓN CORREGIDA
        newHamburger.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('Hamburger clicked - único evento');
            toggleMenu();
        });
        ...
        document.addEventListener('click', function(e) {
            if (menuOpen && !navLinks.contains(e.target) && !newHamburger.contains(e.target)) {
                closeMenu();
            }
        });
...
function initSexyPaceCarousel() {
    ...
    // EVENT LISTENERS DIRECTOS - NO DELEGACIÓN
    if (prevBtn) {
    ...
    // Event listeners para indicadores - DIRECTOS
    indicators.forEach((dot, index) => {
        dot.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
...
// Inicializar sección de nutrición cuando el DOM esté cargado
document.addEventListener('DOMContentLoaded', function() {
    initNutritionSection();
});

function initNutritionSection() {
    console.log('Inicializando sección de nutrición...');
    
    // Gestión de tabs
    initNutritionTabs();
    
    // Galería de fotos
    initNutritionGallery();
    ...
    // Cargar imágenes placeholder si no existen
    replaceNutritionImages();
}

// Funcionalidad de las tabs - CORREGIDA
function initNutritionTabs() {
    ...
    if (!tabs.length || !tabContents.length) {
        console.warn('Tabs de nutrición no encontradas');
        return;
    }
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', function(e) {
            ...
            console.log('Tab clickeada:', targetTab);
            ...
            const targetContent = document.getElementById(`nutrition-${targetTab}`);
            if (targetContent) {
                targetContent.classList.add('active');
                console.log(`Contenido activado: nutrition-${targetTab}`);
                
                // Trigger animaciones si están disponibles
                triggerNutritionAnimations(targetContent);
            } else {
                console.error(`Contenido no encontrado: nutrition-${targetTab}`);
            }
        });
    });
...
function initNutritionGallery() {
    const photos = document.querySelectorAll('.nutrition-photo');
    
    console.log('Fotos de nutrición encontradas:', photos.length);
    ...
    photos.forEach((photo, index) => {
        photo.addEventListener('click', function(e) {
            ...
            if (img) {
                console.log('Click en foto de nutrición:', index);
                
                // Integrar con lightbox existente si está disponible
                if (window.lightbox && typeof window.lightbox.openImage === 'function') {
                    window.lightbox.openImage(img.src, img.alt);
                } else {
                    // Fallback: crear lightbox simple
                    openSimpleLightbox(img.src, img.alt);
                }
            }
        });
...
function initNutritionVideos() {
    const videos = document.querySelectorAll('.nutrition-video');
    
    console.log('Videos de nutrición encontrados:', videos.length);
    ...
    videos.forEach((video, index) => {
        video.addEventListener('click', function(e) {
            ...
            const videoTitle = this.querySelector('.video-info h4');
            const videoText = this.querySelector('.video-info p');
            
            console.log('Click en video de nutrición:', index);
            
            // Crear modal de video
            createVideoModal({
                title: videoTitle ? videoTitle.textContent : 'Video de Nutrición',
                description: videoText ? videoText.textContent : '',
                videoId: `nutrition-video-${index + 1}`,
                thumbnail: this.querySelector('img') ? this.querySelector('img').src : ''
            });
        });
...
// Crear modal de video mejorado
function createVideoModal(videoData) {
    const modal = document.createElement('div');
    modal.className = 'nutrition-video-modal';
    modal.style.cssText = `
        position: fixed;
        top: 0;
        left: 0;
    ...
    modalContent.innerHTML = `
        ...
        <div style="margin-bottom: 20px;">
            <img src="${videoData.thumbnail}" alt="${videoData.title}" style="
                width: 100%;
                max-width: 400px;
                height: 200px;
                object-fit: cover;
        ...
        <h3 style="color: var(--nutrition-dark, #5a6852); margin-bottom: 15px;">
            ${videoData.title}
        </h3>
        
        <p style="color: var(--nutrition-primary, #7a8471); margin-bottom: 25px; line-height: 1.6;">
            ${videoData.description}
        </p>
        
        <div style="background: rgba(157, 180, 145, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <p style="margin: 0; color: var(--nutrition-dark, #5a6852); font-weight: 500;">
                <i class="fas fa-info-circle" style="color: var(--nutrition-accent, #9db491); margin-right: 8px;"></i>
                En una implementación real, aquí se mostraría el reproductor de video integrado.
            </p>
        </div>
        
        <button class="btn btn-primary" onclick="this.closest('.nutrition-video-modal').remove(); document.body.style.overflow = '';" style="
            background: var(--nutrition-accent, #9db491);
            color: white;
            border: none;
            padding: 12px 25px;
            border-radius: 25px;
...
function initNutritionAnimations() {
    ...
    console.log('Elementos para animar:', elementsToAnimate.length);
...
// Trigger animaciones específicas
function triggerNutritionAnimations(container) {
    const animatableElements = container.querySelectorAll('.nutrition-photo, .nutrition-video, .nutrition-article');
...
// Reemplazar imágenes que no se cargan con placeholders
function replaceNutritionImages() {
    const images = document.querySelectorAll('.nutrition-photo img, .nutrition-video img, .nutrition-intro-image img');
    
    images.forEach((img, index) => {
        // Verificar si la imagen ya tiene un error
        if (img.complete && img.naturalHeight === 0) {
            replaceWithPlaceholder(img, index);
        }
        
        // Agregar event listener para errores futuros
        img.addEventListener('error', function() {
            console.log('Imagen no encontrada, usando placeholder:', this.src);
            replaceWithPlaceholder(this, index);
        });
...
// Función mejorada para manejar imágenes faltantes
function initNutritionImages() {
    const nutritionImages = document.querySelectorAll('#nutrition img');
    
    nutritionImages.forEach((img, index) => {
        ...
        testImg.onerror = function() {
            ...
            replaceWithWorkingPlaceholder(img, index);
        };
...
function replaceWithWorkingPlaceholder(img, index) {
    const width = 300;
    const height = 250;
    const text = img.alt || `Imagen ${index + 1}`;
    
    // Usar un placeholder que funcione mejor cross-browser
    const placeholderUrl = `https://picsum.photos/${width}/${height}?random=${index}`;
    
    // Fallback si picsum falla
    img.onerror = function() {
        // Crear un SVG como último recurso
        const svgPlaceholder = `data:image/svg+xml,${encodeURIComponent(`
            <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
                <rect width="100%" height="100%" fill="#e6ddc4"/>
                <text x="50%" y="50%" text-anchor="middle" dy=".3em" fill="#7a8471" font-size="16">${text}</text>
            </svg>
        `)}`;
...
// Manejar errores de carga de imágenes en tiempo real
document.addEventListener('DOMContentLoaded', function() {
...
// Debug: mostrar estado de la sección
function debugNutritionSection() {
    console.log('=== DEBUG SECCIÓN NUTRICIÓN ===');
    console.log('Tabs:', document.querySelectorAll('.nutrition-tab').length);
    console.log('Tab contents:', document.querySelectorAll('.nutrition-tab-content').length);
    console.log('Photos:', document.querySelectorAll('.nutrition-photo').length);
    console.log('Videos:', document.querySelectorAll('.nutrition-video').length);
    console.log('Articles:', document.querySelectorAll('.nutrition-article').length);
    console.log('Pricing sections:', document.querySelectorAll('.nutrition-pricing-section').length);
    
    // Verificar visibilidad
    const nutritionSection = document.getElementById('nutrition');
    if (nutritionSection) {
        const rect = nutritionSection.getBoundingClientRect();
        console.log('Section visible:', rect.top < window.innerHeight && rect.bottom > 0);
