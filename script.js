...
function replaceNutritionImages() {
    ...
    images.forEach((img, index) => {
        // Si la imagen no existe o no carga, usar placeholder
        img.addEventListener('error', function() {
            console.log('Imagen no encontrada, usando placeholder:', this.src);
            
            const width = this.offsetWidth || 300;
            const height = this.offsetHeight || 250;
            const text = this.alt || `Imagen ${index + 1}`;
            
            this.src = `https://via.placeholder.com/${width}x${height}/a8b8a5/ffffff?text=${encodeURIComponent(text)}`;
        });
